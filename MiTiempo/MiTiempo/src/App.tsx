    ...
    const fetchForecast = async () => {
      ...
      try {
        ...
        if (err instanceof Error) {
            ...
            setError("Ocurrió un error desconocido al cargar el pronóstico.");
        }
      } finally {
        setLoading(false);
      }
        ...
        return (
            <div className="text-center p-8 bg-red-50 border border-red-200 rounded-lg">
                ...
                <p className="mt-4 text-xs text-slate-500">
                    Asegúrate de que la clave de la API es correcta y está activa.
                </p>
            </div>
        );
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-sky-200 to-amber-100 text-slate-800">
      ...
      <footer className="text-center p-4 text-xs text-slate-500">
        <p>Datos del tiempo proporcionados por OpenWeatherMap.</p>
      </footer>
    </div>
  );
};
